/**
 * @file libmodbus_master.cc
 * @brief libmodbus Modbus RTU 主站驱动实现。
 *
 * 本文件实现串口 RTU 连接、RS485 模式设置、单总线事务串行化、错误映射和断线快速恢复。
 * 连接失败快速返回，由外层设备或服务周期负责后续重试。
 */
#include <cerrno>
#include <chrono>
...
bool LibModbusMaster::open() {
    ...
    if (!ctx_) {
        ...
        // 尝试开启内核级 RS485 硬件控制模式，使串口驱动自动控制 RTS(DE/RE) 半双工方向。
...
int LibModbusMaster::execute_with_retry(int slave_id, const char* op_name, F&& modbus_func) {
    ...
    // Timeout 属于现场常见通信结果，降级为 debug，避免长时间异常刷写 eMMC。
...
int LibModbusMaster::write_registers(int slave_id, int addr, int count, const uint16_t* vals) {
    ...
    return execute_with_retry(slave_id, "write_registers", [&]() {
        // libmodbus 旧版本接口未声明 const uint16_t*，这里仅适配 ABI，不修改 vals 内容。
