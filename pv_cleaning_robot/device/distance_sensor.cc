// @file distance_sensor.cc
// @brief DistanceSensor 实现：读取 Modbus 距离传感器寄存器并计算诊断数据。
...
/// @brief 构造距离传感器实例，并修正通道数量范围。
DistanceSensor::DistanceSensor(std::shared_ptr<hal::IModbusMaster> modbus, DistanceSensorConfig cfg)
    : modbus_(std::move(modbus)), cfg_(cfg) {
    // 保护性截断：确保通道数在 [1, kDistSensorMaxChannels] 范围内
    cfg_.channel_count = std::clamp(
        cfg_.channel_count, static_cast<uint8_t>(1), static_cast<uint8_t>(kDistSensorMaxChannels));
...
void DistanceSensor::update() {
    ...
    const int ret = modbus_->read_input_registers(cfg_.slave_id,
