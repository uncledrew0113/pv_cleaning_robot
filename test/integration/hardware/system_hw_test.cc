...
// ────────────────────────────────────────────────────────────────────────────
TEST_CASE("System（真实硬件）N=1 完整任务链 + 全程持续采集健康数据",
          "[hw_system][combined]") {
    spdlog::warn("[hw_system][combined] ====================================");
    spdlog::warn("[hw_system][combined] ⚠ 机器人将完整运动一个来回（N=1）！");
    ...
    // 每 500ms 采集一次数据（BMS + HealthService + 喂狗），返回是否超时
    auto poll_once = [&]() {
        f.bms->update();
        f.health->update();
        ...
        auto gd = f.walk_group->get_group_diagnostics();
        auto ld = f.imu->get_latest();
        spdlog::info("[hw_system][combined] record #{}: LT={:.1f} RT={:.1f}rpm "
                     "imu_pitch={:.2f}° state={}",
                     total_health_records,
    ...
    // ── 阶段1：CleanFwd，等待【前端限位】触发 ──────────────────────────────
    f.fsm->dispatch(robot::app::EvScheduleStart{.at_home = true, .passes = 1.0f});
    REQUIRE(f.fsm->current_state() == "CleanFwd");

    spdlog::warn("[hw_system][combined] ★ 机器人正在向前运动，等待【前端限位】触发（最多 {}s）...",
                 hw::kLimitTimeoutSec);

    {
        ...
        while (clock::now() < deadline) {
            if (f.fsm->current_state() == "CleanReturn") break;
            poll_once();
            std::this_thread::sleep_for(500ms);
        }
    }
    ...
    spdlog::info("[hw_system][combined] ✓ 前端限位触发，机器人开始返回（已采集 {} 条）",
                 total_health_records);

    // ── 阶段2：CleanReturn，等待【尾端限位】触发 ──────────────────────────
    spdlog::warn("[hw_system][combined] ★ 机器人正在返回，等待【尾端限位】触发（最多 {}s）...",
                 hw::kLimitTimeoutSec);

    {
        ...
        auto deadline = clock::now() + std::chrono::seconds(hw::kLimitTimeoutSec);
        while (clock::now() < deadline) {
            if (f.fsm->current_state() == "Charging") break;
            poll_once();
            std::this_thread::sleep_for(500ms);
        }
    }
    ...
    spdlog::info("[hw_system][combined] ✓ 尾端限位触发，任务完成（已采集 {} 条）",
                 total_health_records);
    ...
    // CAN 控制帧数增加（说明 MotionService 全程在发帧）
    const uint32_t frames_after = f.walk_group->get_group_diagnostics().ctrl_frame_count;
    spdlog::info("[hw_system][combined] CAN 帧: {} → {} (增量={})",
                 frames_before, frames_after, frames_after - frames_before);
    CHECK(frames_after > frames_before + 20u);

    // 无故障
    CHECK(f.dispatched_faults.empty());

    // JSONL 存在，行数与采集次数一致，每行合法 JSON
    REQUIRE(std::filesystem::exists(hw::kHealthJsonlPath));
    {
        std::ifstream ifs(hw::kHealthJsonlPath);
        REQUIRE(ifs.is_open());
        ...
        spdlog::info("[hw_system][combined] JSONL 已写入 {} 行，路径: {}",
