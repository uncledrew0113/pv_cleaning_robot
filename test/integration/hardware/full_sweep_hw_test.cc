 ...
 *   [hw_sweep][limit_stop]   - 手动触发前限位：验证 GPIO → 回调 → emergency_override 全链路
 *   [hw_sweep][limit_stop][rear] - 手动触发后限位：同上，针对后限位（REAR）
 *
 * 硬件接线要求：
...
TEST_CASE("[hw_sweep][with_pid] 带 PID 全扫描序列", "[hw_sweep][with_pid]") {
    ...
    spdlog::info(
        "[hw_sweep][with_pid] 开始清扫：{:.1f} RPM，PID 目标航向 {:.2f}°", kp.sweep_rpm, init_yaw);
