...
// ═══════════════════════════════════════════════════════════════════════════
//  Part 5: BMS 设备层 Modbus 路径测试
// ═══════════════════════════════════════════════════════════════════════════

TEST_CASE("BMS Modbus - protocol_type() 返回 kModbus", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    device::BMS bms(mock);
    CHECK(bms.protocol_type() == device::BMS::ProtocolType::kModbus);
}

TEST_CASE("BMS Modbus - Modbus 打开失败时 open() 返回 false", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
...
TEST_CASE("BMS Modbus - 正常 open() 后数据有效", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.soc_raw = 750u;            // 75.0%
    p.total_voltage_raw = 576u;  // 57.6V
    p.cell_count = 16u;
    p.ntc_count = 3u;
    setup_status(*mock, p);
    setup_temps(*mock, {65u, 72u, 68u});  // 25/32/28°C
    ...
    CHECK(data.voltage_v == Approx(57.6f).margin(0.01f));
}

TEST_CASE("BMS Modbus - 充电电流：符号翻转为正=充电", "[device][bms2]") {
    // raw=29500 → protocol current=-50A（充电方向）→ device current_a=+50A（正=充电）
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.current_raw = 29500u;
    setup_status(*mock, p);
    setup_temps(*mock, {65u, 65u, 65u});

    device::BMS bms(mock);
    REQUIRE(bms.open() == device::DeviceError::OK);

    auto data = bms.get_data();
    CHECK(data.current_a == Approx(50.0f).margin(0.01f));
    CHECK(data.charging);
}

TEST_CASE("BMS Modbus - 放电电流：device current_a 为负", "[device][bms2]") {
    // raw=30200 → protocol current=+20A（放电）→ device current_a=-20A
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.current_raw = 30200u;
    setup_status(*mock, p);
    ...
    auto data = bms.get_data();
    CHECK(data.current_a == Approx(-20.0f).margin(0.01f));
    CHECK_FALSE(data.charging);
}

TEST_CASE("BMS Modbus - 低电量告警", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.soc_raw = 100u;  // 10.0% < 默认 low_soc=15%
    setup_status(*mock, p);

    device::BMS bms(mock, 0x51u, 95.0f, 15.0f);
    REQUIRE(bms.open() == device::DeviceError::OK);
...
TEST_CASE("BMS Modbus - 充满检测（需 kFullChargeConfirm=6 次连续确认）", "[device][bms2]") {
    // SOC=96%（≥ full_soc=95%），idle 电流（raw=30000 → 0A，|I|<0.5A）
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.soc_raw = 960u;        // 96.0%
    p.current_raw = 30000u;  // 0A
    setup_status(*mock, p);

    device::BMS bms(mock, 0x51u, 95.0f, 15.0f);
    REQUIRE(bms.open() == device::DeviceError::OK);  // open() 内已读 1 次，full_charge_count=1

    // 再读 5 次凑满 kFullChargeConfirm=6
    for (int i = 0; i < 5; ++i)
        bms.update();
...
TEST_CASE("BMS Modbus - alarm_flags 从 Bms2AlarmFlags 映射", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    setup_status(*mock, p);

    // 设置告警：单体过压=1，短路保护
    // reg[0] 低字节 bit[2:0]=1 → 0x0001（cell_overvolt_level）
    // reg[2] 低字节 bit6=1 → 0x0040（short_circuit）
    mock->alarm_regs[0] = 0x0001u;
    mock->alarm_regs[2] = 0x0040u;
    ...
    CHECK((data.alarm_flags & device::BMS::PROT_SHORT_CIRCUIT) != 0u);
    CHECK((data.alarm_flags & device::BMS::PROT_BAT_UNDERVOLT) == 0u);
}

TEST_CASE("BMS Modbus - alarm_detail2 完整告警结构写入诊断", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    setup_status(*mock);
    // AFE 通信故障：reg[5] 高字节 bit1 → reg[5]=0x0200
    mock->alarm_regs[5] = 0x0200u;

    device::BMS bms(mock);
    REQUIRE(bms.open() == device::DeviceError::OK);

    auto diag = bms.get_diagnostics();
    CHECK(diag.alarm_detail2.afe_comm_fault);
}

TEST_CASE("BMS Modbus - 每 4 次 update() 触发电芯电压读取", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.cell_count = 4u;
    p.ntc_count = 2u;
    setup_status(*mock, p);
    setup_temps(*mock, {65u, 70u});
    // 4 节电芯：3600 / 3650 / 3580 / 3630 mV
    setup_cells(*mock, {3600u, 3650u, 3580u, 3630u});

    device::BMS bms(mock);
    REQUIRE(bms.open() == device::DeviceError::OK);  // update_cycle_=0（open 不递增）

    // 前 3 次 update 不读电芯
    for (int i = 0; i < 3; ++i)
        bms.update();

    // 第 4 次 update_cycle_=4，触发 read_cell_voltages_modbus()
    bms.update();

    auto diag = bms.get_diagnostics();
    CHECK(diag.cell_voltages2.count == 4u);
    CHECK(diag.cell_voltages2.mv[0] == 3600u);
    CHECK(diag.cell_voltages2.mv[1] == 3650u);
    CHECK(diag.cell_voltages2.mv[2] == 3580u);
    CHECK(diag.cell_voltages2.mv[3] == 3630u);
}

TEST_CASE("BMS Modbus - NTC 温度从温度块读取", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.ntc_count = 3u;
    setup_status(*mock, p);
    // 25 / 30 / 35°C
    setup_temps(*mock, {65u, 70u, 75u});
    ...
    auto data = bms.get_data();
    // 最高温度 = 35°C（来自 NTC 块）
    CHECK(data.temperature_c == Approx(35.0f).margin(0.1f));

    auto diag = bms.get_diagnostics();
    CHECK(diag.ntc_count == 3u);
    CHECK(diag.ntc_temps_c[0] == Approx(25.0f).margin(0.1f));
    CHECK(diag.ntc_temps_c[1] == Approx(30.0f).margin(0.1f));
    CHECK(diag.ntc_temps_c[2] == Approx(35.0f).margin(0.1f));
}

TEST_CASE("BMS Modbus - NTC 读取失败时退化为 BasicInfo 统计最高温", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.ntc_count = 3u;
    p.temp_max_raw = 80u;  // 80-40 = 40°C
    setup_status(*mock, p);
    // 故意让温度块读取失败
    mock->temp_fail = true;
    ...
    auto data = bms.get_data();
    // 退化到 BasicInfo 中的 temp_max_c = 40°C
    CHECK(data.temperature_c == Approx(40.0f).margin(0.1f));
}

TEST_CASE("BMS Modbus - power_w / energy_wh 写入诊断", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    Bms2StatusParams p;
    p.power_w = 2400u;
    p.energy_wh = 50000u;
    ...
    auto diag = bms.get_diagnostics();
    CHECK(diag.power_w == Approx(2400.0f).margin(0.5f));
    CHECK(diag.energy_wh == Approx(50000.0f).margin(0.5f));
    CHECK(diag.nominal_capacity_ah == Approx(0.0f).margin(0.01f));  // BMS2 无标称容量
}

TEST_CASE("BMS Modbus - 主状态块读取失败 → error_count 递增", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    setup_status(*mock);
    REQUIRE(std::make_shared<device::BMS>(mock)->open() == device::DeviceError::OK);

    // 重新创建 BMS，此次把主状态块标记为失败
    mock->status_fail = true;
    device::BMS bms(mock);
    ...
    auto diag = bms.get_diagnostics();
    CHECK(diag.error_count >= 4u);
}

TEST_CASE("BMS Modbus - update_count 反映成功读取次数", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    setup_status(*mock);

    device::BMS bms(mock);
    REQUIRE(bms.open() == device::DeviceError::OK);  // open() 内 1 次成功 → update_count=1
    ...
    CHECK(diag.error_count == 0u);
}

TEST_CASE("BMS Modbus - mos_control() 始终返回 false（仅读协议）", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    setup_status(*mock);
...
TEST_CASE("BMS Modbus - get_data/get_diagnostics 线程安全", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    setup_status(*mock);
    setup_temps(*mock, {65u, 70u, 68u});
    ...
    std::thread updater([&] {
        for (int i = 0; i < 30 && not stop.load(); ++i) {
            bms->update();
...
TEST_CASE("BMS Modbus - close() 后 is_open 为 false", "[device][bms2]") {
    auto mock = std::make_shared<BmsMockModbus2>();
    setup_status(*mock);
    ...
    bms.close();
    CHECK_FALSE(mock->opened);
}

// ═══════════════════════════════════════════════════════════════════════════
//  Part 6: BMS2 硬件集成测试（需接真实 BMS2 设备）
// ═══════════════════════════════════════════════════════════════════════════

TEST_CASE("BMS2 集成测试 - Modbus 连接与数据读取", "[integration][bms2]") {
    using namespace std::chrono_literals;

    // 硬件参数（经 bms2_probe.sh 扫描确认）：
    //   /dev/ttyS8 slave=0x01 有响应（其他端口/地址无响应）
    // 注意：BMS1（嘉佰达通用协议 V4）也在 ttyS8，该电池同时支持 Modbus RTU
    const char* port = "/dev/ttyS8";
    const uint8_t slave_id = 0x01;

    hal::ModbusConfig cfg;
    cfg.baudrate = 9600;
    cfg.parity = 'N';
    cfg.data_bits = 8;
    cfg.stop_bits = 1;

    auto modbus = std::make_shared<driver::LibModbusMaster>(port, cfg);
    device::BMS bms(modbus, slave_id, 95.0f, 15.0f);

    SECTION("Modbus 连接与首次数据读取") {
        bool opened = (bms.open() == device::DeviceError::OK);
        REQUIRE(opened);
        spdlog::info("[BMS2 集成] Modbus 打开成功，port={} slave={:#04x}", port, slave_id);

        auto data = bms.get_data();
        if (data.valid) {
            spdlog::info("[BMS2 集成] SOC={:.1f}%  电压={:.2f}V  电流={:.3f}A",
                         data.soc_pct,
                         data.voltage_v,
                         data.current_a);
        }
    ...
    SECTION("基本数据合理性验证") {
        ...
        spdlog::info("[BMS2 集成] SOC={:.1f}%  电压={:.2f}V  电流={:.3f}A  温度={:.1f}℃",
                     data.soc_pct,
                     data.voltage_v,
                     data.current_a,
                     data.temperature_c);
        spdlog::info("[BMS2 集成] 充电={}  充满={}  低电量={}  告警=0x{:04X}",
                     data.charging,
                     data.fully_charged,
                     data.low_battery,
                     data.alarm_flags);
    ...
    SECTION("电芯电压读取（4 次 update 触发 Modbus FC03）") {
        REQUIRE(bms.open() == device::DeviceError::OK);
        ...
        auto diag = bms.get_diagnostics();
        spdlog::info("[BMS2 集成] 串联节数={}  最高={:.3f}V  最低={:.3f}V",
                     diag.cell_count,
                     diag.cell_voltage_max_v,
                     diag.cell_voltage_min_v);

        if (diag.cell_voltages2.count > 0u) {
            CHECK(diag.cell_voltage_max_v > 0.0f);
            CHECK(diag.cell_voltage_max_v >= diag.cell_voltage_min_v);
            for (uint8_t i = 0; i < diag.cell_voltages2.count; ++i) {
                spdlog::info("[BMS2 集成]   cell[{}] = {} mV", i, diag.cell_voltages2.mv[i]);
            }
        }
    }
    ...
    SECTION("NTC 温度读取") {
        ...
        auto diag = bms.get_diagnostics();
        spdlog::info("[BMS2 集成] NTC 探头数={}", diag.ntc_count);
        for (uint8_t i = 0; i < diag.ntc_count; ++i) {
            spdlog::info("[BMS2 集成]   NTC[{}] = {:.1f}°C", i, diag.ntc_temps_c[i]);
    ...
    SECTION("告警 / 故障状态读取") {
        ...
        auto diag = bms.get_diagnostics();
        spdlog::info("[BMS2 集成] alarm_flags=0x{:04X}", diag.alarm_flags);
        spdlog::info("[BMS2 集成] afe_chip_fault={}  eeprom_fault={}  rtc_fault={}",
                     diag.alarm_detail2.afe_chip_fault,
                     diag.alarm_detail2.eeprom_fault,
                     diag.alarm_detail2.rtc_fault);
        // 基本验证：正常运行时不应有 AFE 故障
        // 可根据实际硬件状态放宽此断言
        SUCCEED();
    }
    ...
    SECTION("持续 10 次 update 无崩溃，通信错误率低于 40%") {
        ...
        auto diag = bms.get_diagnostics();
        spdlog::info(
            "[BMS2 集成] update_count={} error_count={}", diag.update_count, diag.error_count);
    ...
    SECTION("完整诊断数据打印") {
        ...
        spdlog::info("  功率          = {:.1f} W", diag.power_w);
        spdlog::info("  能量          = {:.1f} Wh", diag.energy_wh);
