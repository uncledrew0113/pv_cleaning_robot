...
struct add_unique {
    using type = conditional_t<is_base_of<type_wrapper<T>, Accumulator>::value,
                               Accumulator,
...
struct unique_impl<type_wrapper<UniqueTypes...>, Head, Tail...> {
   private:
    ...
    using next_uniques = conditional_t<is_base_of<type_wrapper<Head>, current_set>::value,
...
struct is_unique<type_wrapper<Rs...>, T, Ts...>
    : conditional_t<is_base_of<type_wrapper<T>, inherit<type_wrapper<Rs>...>>::value,
                    false_type,
...
struct transitions_sub<sm<Tsm>> {
    ...
    template <class, class SM, class TDeps, class TSubs>
    constexpr static bool execute(const anonymous &,
                                  SM &,
                                  TDeps &deps,
                                  TSubs &subs,
                                  typename SM::state_t &) {
...
struct conditional_mapping<true, aux::type_wrapper<T1, aux::inherit<Rs...>>, T2, Ts...> {
    using type =
        unique_mappings_impl<aux::type_wrapper<aux::inherit<T1>, extend_mapping_t<T2, Rs...>>,
                             Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct conditional_mapping<false, aux::type_wrapper<T1, aux::inherit<Rs...>>, T2, Ts...> {
    using type = unique_mappings_impl<
        aux::type_wrapper<aux::inherit<T1, aux::type_wrapper<typename T2::element_type>>,
                          aux::inherit<T2, Rs...>>,
        Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct unique_mappings_impl<aux::type_wrapper<T1, aux::inherit<Rs...>>, T2, Ts...>
    : conditional_mapping<aux::is_base_of<aux::type_wrapper<typename T2::element_type>, T1>::value,
...
template <class... Ts>
struct unique_mappings
    : unique_mappings_impl<aux::type_wrapper<aux::none_type, aux::inherit<>>, Ts...> {};
template <class T>
...
struct get_state_mapping {
    ...
    struct combined_mapping {
        ...
        constexpr static bool execute(const TEvent &event,
            ...
            return state_mapping::template execute<TEvent, TSm, TDeps, TSubs>(
                       event, sm, deps, subs, current_state) ||
                   any_state_mapping::template execute<TEvent, TSm, TDeps, TSubs>(
                       event, sm, deps, subs, current_state);
        }
    };
...
struct jump_table {
    template <int,
              ...
              class TDeps,
              class TSubs>
    constexpr static bool dispatch(sm_impl &,
                                   State &,
                                   const TEvent &,
                                   TDeps &,
                                   TSubs &,
...
constexpr void log_action(const aux::type_wrapper<no_policy> &,
                          TDeps &,
                          const TAction &,
                          const TEvent &) {}
...
constexpr void log_guard(const aux::type_wrapper<no_policy> &,
                         TDeps &,
                         const TGuard &,
                         const TEvent &,
                         bool) {}
...
struct sm_impl : aux::conditional_t<aux::should_not_subclass_statemachine_class<Tsm>::value,
    ...
    constexpr bool process_event(const TEvent &event, TDeps &deps, TSubs &subs) {
        ...
        do {
            do {
                ...
                deps, subs, changed, aux::type_wrapper<defer_queue_t<TEvent>>{}, events_t{}));
        } while (process_queued_events(
            deps, subs, queued_handled, aux::type_wrapper<process_queue_t<TEvent>>{}, events_t{}));
    ...
    template <class TEvent,
              ...
              __BOOST_SML_REQUIRES(aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value &&
                                   !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value)>
    constexpr bool process_internal_event(const TEvent &event,
                                          TDeps &deps,
                                          TSubs &subs,
                                          state_t &current_state) {
...
constexpr decltype(auto) get_arg(const aux::type_wrapper<back::defer<TEvents...>> &,
                                 const TEvent,
                                 Tsm &sm,
                                 TDeps &) {
...
template <class... TEvents, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type_wrapper<back::process<TEvents...>> &,
                                 const TEvent,
                                 Tsm &sm,
                                 TDeps &) {
...
struct call<TEvent, aux::type_list<action_base>, TLogger> {
    template <class T, class Tsm, class TDeps, class TSubs>
    constexpr static auto execute(T object,
                                  const TEvent &event,
                                  Tsm &sm,
                                  TDeps &deps,
                                  TSubs &subs) {
...
constexpr void update_composite_states(TSubs &subs,
                                       aux::true_type,
...
constexpr void update_current_state(SM &,
                                    ...
                                    const TSrcState &,
                                    const TDstState &) {
    back::policies::log_state_change<typename SM::sm_t>(aux::type_wrapper<typename SM::logger_t>{},
                                                        deps,
