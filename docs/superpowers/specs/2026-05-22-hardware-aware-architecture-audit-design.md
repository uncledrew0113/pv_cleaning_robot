...
以下决策作为后续审计和改造建议的业务基准：

1. P1 故障处理
   - 先停止当前清扫动作。
   - 再无刷返航到停机侧。
   - 故障码必须上报云端，返航只是处置动作，不应吞掉故障事实。
2. RPC start 语义
   - 不要求机器人必须位于停机位。
   - 只要电池电量满足、设备自检通过、当前位置/运动条件满足，就开启一次清扫任务。
   - 这是一种明确的人工/云端特权启动能力，不应被计划任务的停机位约束误伤。
3. 限位触发语义
   - 限位触发后只需要急停。
   - 后续正向清扫、反向清扫或返航动作由 FSM 根据当前状态决定。
   - 不需要在 SafetyMonitor 层自动反向脱离限位。
4. BMS 无数据策略
   - 正式环境：BMS 无数据禁止启动。
   - 测试环境：允许降级运行，但必须能在日志/遥测中看出是降级。
5. 视觉 PID 定位
   - 视觉 PID 只作为辅助纠偏。
   - 不参与安全相关决策，不作为位置有效性的唯一依据。
6. FSM 后续目标
   - 当前 FSM 以完整来回清扫为基准，不覆盖 0.5 趟清扫业务。
   - 后续需要优化 FSM，使其支持 0.5 趟模式，并提升状态稳定性、异常输入鲁棒性和限位事件处理健壮性。
7. 云端上报基线
   - fault 和自检失败必须能上报云端。
   - 云端至少应能区分故障等级、故障码、故障描述、自检失败原因和当前处置动作。
...
- RPC start 明确允许从非停机位启动一次清扫任务。
- 启动前仍必须满足电池电量、自检、位置/运动安全条件。
- 审计重点不是收紧为停机位启动，而是把 RPC start 与计划任务启动的不同准入规则表达清楚。
...
- 根据用户确认，SafetyMonitor 只负责急停，不负责反向脱离限位；下一步动作必须由 FSM 决定。
...
- P0 是硬急停语义。
- P1 的目标语义已确认：先停止，再无刷返航到停机侧，同时保留故障上报。
- 当前 app 逻辑表现为进入 Fault 并急停，与 P1 目标语义不一致，需要作为高优先级业务风险。
...
- 审计需要区分正式环境与测试环境：正式环境 BMS 无数据禁止启动，测试环境允许降级运行但必须可观测。
...
- fault、自检失败、任务拒绝原因必须有清晰云端事件或遥测字段，不能只依赖本地日志。
...
- 审计应明确 stop 后的恢复路径：允许直接 start，还是必须 reset/return 后再启动。
...
- P1 应先停止，再无刷返航到停机侧。
- P2 的目标语义需要与代码统一。
- 所有 P0/P1/P2 故障至少应上报等级、故障码、描述和时间。

风险：

- P1 当前实现与目标业务语义不一致，是直接影响硬件处置策略的风险：目标是先停机，再无刷返航。
- P2 若声明为降速继续但没有实际降速路径，会误导云端和运维判断。
- `FaultService` 故障清除和线程安全接口需要审计，否则 reset 后可能残留故障状态。
- 当前云端业务遥测缺少 FaultService 故障码字段，无法让平台直接看到 app 级故障原因。
...
- RPC start 是明确的特权启动入口，可从非停机位启动，但仍必须通过电池和自检。
...
1. P1 故障语义不一致：目标语义是先停机再无刷返航，当前 app 路径倾向 Fault + emergency stop。
...
9. RPC start 的非停机位启动能力需要独立准入规则，不能复用计划任务的停机位规则。
10. BMS/IMU/GPS 非致命失败策略需要按生产和测试场景分开定义。
11. 当前业务遥测缺少 app 级故障码、自检失败原因和处置动作字段，云端只能间接看到部分状态。
12. 当前 FSM 缺少 0.5 趟清扫模式，完整往返模型无法表达“只清扫单程/半趟”的业务。
...
1. 先修正 P1/P2 故障策略、fault/self-check 云端上报、RPC start 准入规则、限位/parking_side 语义。
2. 再把领域基础类型抽到独立 domain 层，例如 `ParkingSide`、`TaskPhase`、`LimitEndpoint`、`FaultSeverity`。
3. 再拆云控到 app 的反向依赖，引入窄接口或命令端口。
4. 再优化 FSM 模型，支持完整趟和 0.5 趟模式，并明确异常限位事件的处理策略。
5. 最后用 CMake 目标或 include 规则约束层级，避免跨层依赖回流。

## Current Code Observations To Confirm In Audit

基于当前代码初查，正式审计需要验证并形成最终结论：

1. `FaultService::report()` 当前保存 `FaultEvent{level, code, description, timestamp_ms}` 并发布 EventBus，但 ThingsBoard 上报没有直接订阅 FaultEvent。
2. `FaultHandler` 当前对 P0/P1 都执行 `motion_->emergency_stop()`，P1 没有按目标语义无刷返航。
3. `RobotSupervisor::snapshot()` 当前没有 fault 字段，`ThingsBoardJsonCodec::build_business_telemetry()` 当前只输出 `device_state`、`task_state`、`active_config_version`。
4. `HealthService` 会上报部分硬件级故障布尔值和设备错误码，例如行走电机/滚刷/BMS 告警，但不等于 app 级 FaultService 故障码上报。
5. 启动位置异常会通过 `publish_status_event("startup_position_invalid", ...)` 上报部分原因。
6. FSM 内部自检失败主要写日志并回到 Idle；RPC start 拒绝会返回 reason，但计划启动/内部自检失败缺少统一云端事件。
