 ...
 * GPIO 硬件抽象接口。
 *
 * 同一接口覆盖输入边沿监控和输出 DO 控制。安全相关输入可配置实时优先级、
 * CPU 亲和性和软件去抖；不支持硬件 IRQ 的 GPIO 控制器可强制使用轮询模式。
 */
/**
 * @file i_gpio_pin.h
 * @brief GPIO 引脚 HAL 抽象接口。
 *
 * 本接口统一输入、输出、边沿回调和监控线程配置，供主限位、姿态限位和锁止电机复用。
 */
#pragma once
#include <functional>
...
struct GpioConfig {
    ...
    int debounce_ms{0};    ///< 软件去抖时间（毫秒），0 表示不开启去抖。
...
///
/// 生产实现为 driver/libgpiod_pin（基于 libgpiod v1.6）。
class IGpioPin {
   public:
    ...
    /// @brief 读取当前引脚电平，true 表示高电平。
    virtual bool read_value() = 0;

    /// @brief 输出高/低电平，仅输出模式有效，true 表示高电平。
    virtual bool write_value(bool high) = 0;

    /// @brief 注册边沿回调，仅输入模式下 start_monitoring() 后触发。
    /// @note 回调在专用线程中执行，严禁在回调中执行长耗时或阻塞操作。
